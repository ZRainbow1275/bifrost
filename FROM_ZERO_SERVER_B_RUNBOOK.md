...
如果这台 Server B 是刚刚重装过的，或者这个 IP 以前连过别的系统，Windows 可能会报：
...
这说明 Windows 还记着旧系统的 SSH 指纹。确认你确实刚重装过 Server B 后，在 Windows PowerShell 里执行：

```powershell
ssh-keygen -R <SERVER_B_IP>
ssh-keygen -R "[<SERVER_B_IP>]:22"
ssh-keygen -R "[<SERVER_B_IP>]:22222"
ssh-keygen -R "[<SERVER_B_IP>]:11964"
```
...
如果这台机器虽然是海外 VPS，但第一次 `git clone` 还是报 `GnuTLS recv error (-110)` 或 `SSL connection timeout`，先不要反复重试。
因为这时仓库还没拉下来，不能先用 `./install.sh --github-hosts-repair`，要先跳到下面的 `3.1`，把 `/etc/hosts` 手动修好，再回到这里重试 clone。
...
如果你刚把 Server B 重装过，或者这台机器的 SSH 指纹变了，Windows 里可能会先报：
