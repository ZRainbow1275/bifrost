...
backup_config() {
    ...
    # Get encryption key first so the key file itself is included in the backup.
    local enc_key
    ...
    staging_dir="$(mktemp -d /tmp/ai-gateway-bridge-backup.XXXXXX)"
...
setup_auto_backup() {
    ...
    local script_path
    local backup_log_dir="${LOG_DIR:-/var/log/ai-gateway-bridge}"
