...
# 注意: 本目录为历史快照，当前主实现以仓库根目录的 Bifrost `install.sh` 为准
# =============================================================================
...
readonly LEGACY_NOTICE="历史快照目录，当前主实现请使用仓库根目录的 install.sh"
...
# --- Main Menu ---
show_legacy_notice() {
    log_warn "${LEGACY_NOTICE}"
    log_warn "当前 authoritative 文档与测试入口位于仓库根目录的 docs/0325/* 与 tests/test-in-docker.sh"
}
...
注意:
  本目录为历史快照。当前主实现、最新审计结果、统一测试入口与部署文档
  以仓库根目录的 install.sh / docs/0325/* / tests/test-in-docker.sh 为准。
