#!/usr/bin/env bash
###############################################################################
# AI Gateway Bridge - Backup & Restore Module
#
...
# Constants
# =============================================================================
BACKUP_BASE_DIR="/var/backups/ai-gateway-bridge"
BACKUP_ENCRYPTION_KEY_FILE="/root/.ai-gateway-bridge-backup-key"
BACKUP_MAX_KEEP=7
...
WHITELIST_INSTALLED="/opt/ai-gateway-bridge/configs/whitelist"
...
# The key is stored at /root/.ai-gateway-bridge-backup-key with mode 600.
...
_collect_config_paths() {
    local paths=(
        ...
        "/etc/wireguard"
        "/etc/ai-gateway-bridge/vpn"
        # User registry (credentials, guides)
        "/etc/ai-gateway-bridge/users"
        # Server B deploy state (generated by server-b.sh)
        "/root/.ai-gateway-bridge"
        # Anti-DPI dest pool
        "/opt/ai-gateway-bridge/dest-pool.txt"
...
# backup_config()
#
# Create an encrypted tar.gz archive of all AI Gateway Bridge configs.
# Archives are stored under /var/backups/ai-gateway-bridge/ with a timestamp.
...
backup_config() {
    ...
    # Write manifest
    {
        echo "# AI Gateway Bridge Backup Manifest"
...
setup_auto_backup() {
    ...
    if [[ ! -f "${script_path}" ]]; then
        ...
        script_path="/opt/ai-gateway-bridge/scripts/backup.sh"
    fi

    local cron_entry="0 3 * * * /usr/bin/env bash ${script_path} backup >> /var/log/ai-gateway-bridge/backup-cron.log 2>&1"
    local cron_marker="# ai-gateway-bridge-daily-backup"
...
manage_backups() {
    while true; do
        echo ""
        echo -e "${BLUE}============================================${NC}"
        echo -e "${BLUE}  AI Gateway Bridge - Backup Management     ${NC}"
        echo -e "${BLUE}============================================${NC}"
        echo ""
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
    case "${1:-}" in
        ...
        rotate-ip)
            ...
            ;;
        help|--help|-h)
            echo "AI Gateway Bridge - Backup Management"
            echo ""
            echo "Usage:"
