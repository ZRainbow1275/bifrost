...
BACKUP_BASE_DIR="${BACKUP_BASE_DIR:-/var/backups/bifrost}"
BACKUP_ENCRYPTION_KEY_FILE="${BACKUP_ENCRYPTION_KEY_FILE:-/root/.bifrost-backup-key}"
BACKUP_MAX_KEEP="${BACKUP_MAX_KEEP:-7}"
...
NEW_API_DIR="${NEW_API_DIR:-/opt/new-api}"
INSTALL_DIR="${INSTALL_DIR:-/opt/bifrost}"
SECURITY_STATE_DIR="${SECURITY_STATE_DIR:-/etc/bifrost}"
WHITELIST_INSTALLED="${WHITELIST_INSTALLED:-/opt/bifrost/configs/whitelist}"
FAIL2BAN_CONFIG="${FAIL2BAN_CONFIG:-/etc/fail2ban}"
SYSCTL_HARDENING="${SYSCTL_HARDENING:-/etc/sysctl.d/99-ai-gateway-hardening.conf}"
SERVER_B_CONF="${SERVER_B_CONF:-/root/server-b-connection.conf}"
CONNECTION_INFO="${CONNECTION_INFO:-/root/ai-gateway-connection.txt}"
